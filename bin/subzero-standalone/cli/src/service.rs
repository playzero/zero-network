...
use subzero_standalone_runtime::RuntimeApi;
...
pub fn create_extrinsic(
	...
	function: impl Into<subzero_standalone_runtime::Call>,
	nonce: Option<u32>,
) -> subzero_standalone_runtime::UncheckedExtrinsic {
	...
	let period = subzero_standalone_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: subzero_standalone_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<subzero_standalone_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<subzero_standalone_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<subzero_standalone_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<subzero_standalone_runtime::Runtime>::new(),
		frame_system::CheckEra::<subzero_standalone_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<subzero_standalone_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<subzero_standalone_runtime::Runtime>::new(),
		pallet_asset_tx_payment::ChargeAssetTxPayment::<subzero_standalone_runtime::Runtime>::from(tip, None),
	);

	let raw_payload = subzero_standalone_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			subzero_standalone_runtime::VERSION.spec_version,
			subzero_standalone_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	subzero_standalone_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		subzero_standalone_runtime::Signature::Sr25519(signature),
		extra,
	)
}
...
mod tests {
	...
	use subzero_standalone_runtime::{
