...
pub fn create_extrinsic(
	...
	function: impl Into<alphaville_runtime::RuntimeCall>,
	nonce: Option<u32>,
	...
	let period = alphaville_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: alphaville_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<alphaville_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<alphaville_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<alphaville_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<alphaville_runtime::Runtime>::new(),
		frame_system::CheckEra::<alphaville_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<alphaville_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<alphaville_runtime::Runtime>::new(),
		pallet_asset_tx_payment::ChargeAssetTxPayment::<alphaville_runtime::Runtime>::from(tip, None),
	);

	let raw_payload = alphaville_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			alphaville_runtime::VERSION.spec_version,
			alphaville_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	alphaville_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		alphaville_runtime::Signature::Sr25519(signature),
		extra,
	)
}
